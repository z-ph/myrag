...
财务处知识库系统：文档管理 + RAG 智能问答。文档（含图片 OCR）上传后分块，同时写入向量索引、PostgreSQL BM25 倒排索引和 Neo4j 知识图谱。问答由模型按需调用检索工具，底层执行向量、稀疏、图谱三路召回并使用 cross-encoder 重排；用户可见来源由本轮实际查阅的文档自动生成。实现见 `docs/langchain-alignment.md`。
...
| 文档列表（含文件名/正文搜索，以及类型、状态、上传年份筛选） | `GET /documents` |
...
| `GET /documents`、`GET /documents/{id}/file`、`GET /conversations/{id}/images/{filename}` | ✅ | ✅ | ✅ | ✅ | ✅ |
