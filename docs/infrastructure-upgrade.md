...
- **配置无容错（必填校验）**:`VITE_BASE` 缺失或格式错误在 vite.config 加载时直接 throw（dev/构建均校验）；`BACK_END_URL` 仅 dev 代理使用，只在 `command === 'serve'` 时校验（生产构建由 nginx 反代，不经过 vite，**不进 docker 构建链路**）。main.tsx 直接用 define 注入值，无 `??` 兜底。`loadEnv` 需传空前缀读全部变量、envDir 用 `process.cwd()` 定位仓库根（vite 打包配置到临时目录导致 `import.meta.dirname` 偏移）。
